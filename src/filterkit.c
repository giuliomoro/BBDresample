...
FLOAT FilterUD( HWORD Imp[], HWORD ImpD[],
		     UHWORD Nwing, BOOL Interp,
		     HWORD Xp, HWORD Ph, HWORD Inc, UHWORD dhb, FLOAT X[], HWORD length)
{
    HWORD a;
    HWORD *Hp, *Hdp, *End;
    FLOAT v, t;
    ...
    if (Interp)
	  ...
	  t += (((WORD)*Hdp)*a)>>Na; /* t is now interp'd filter coeff */
      t/=32768.0;
	  t *= X[Xp%length];		/* Mult coeff by input sample */
	  v += t;			/* The filter output */
	  Ho += dhb;	    /* IR step */
	  Xp += Inc+add;		/* Input signal step. NO CHECK ON BOUNDS */
      }
    else 
      while ((Hp = &Imp[Ho>>Na]) < End) {
	  t = *Hp;		/* Get IR sample */
...
double zerox(HWORD *Data, double Factor)
{
   ...
   for (i=0; i<MAXITER; i++)
      {
      x = (hi+lo)/2.0;
      v  = FilterUD(Imp,ImpD,Nwing,TRUE,0,  (HWORD)(x*Pmask),    -1,dhb,(FLOAT *)Data,0);
      v += FilterUD(Imp,ImpD,Nwing,TRUE,1,(HWORD)((1-x)*Pmask), 1,dhb,(FLOAT *)Data,0);
      v >>= Nhg;
      v *= LpScl;
