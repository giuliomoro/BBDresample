...
FLOAT FilterUD(HWORD Imp[], /*pointer to a table of filter values*/
                HWORD ImpD[], /*poiner to a table of differences of filter values*/
                UHWORD Nwing,/*length of the tables*/
                BOOL Interp,/*use linear interpolation between filter values*/
                HWORD Xp, /*index in the input signal*/
                HWORD Ph, /*location between samples in the input signal*/
                HWORD Inc, /*-1 performs left wing, +1 performs right wing*/
                UHWORD dhb,/*step in the filter impulse response*/
                FLOAT X[],/*array containing the input signal*/
                HWORD length);/*length of the input array to be used as circular buffer. 
                              If this is 0 doesn't use circular buffer*/
