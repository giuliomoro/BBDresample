...
\n\
      resample [-noFilterInterp] [-f filterFile] [-terse] inputSoundFile outputSoundFile\n\
      resample [options as above] inputSoundFile outputSoundFile\n\
      resample -version\n\
      -p NUM preset, one of:\n\
                    0: slow, 1:chor1, 2:chor2, 3:fast,\n\
                    4:slow wet only, 5:fast wet only\n\
                    6: dry only\n\
                    7:ext lfo wet only, 8:ext lfo 1:1 mix \n\
                    external lfo has to be provided in the right channel of inputSoundFile.\n\
      -w NUM modulation width] \n\
      -c clock frequency of the BBD\n\
      -b BBD only, bypasses filters\n\
\n\
...
int main(int argc, char *argv[])
{
    ...
    case 'w':			       /* -width lfo*/
	    if (--argc)
	      sscanf(*(++argv),"%lf",&modwidth);
	    if (trace)
	      printf("Lfo width set to %f.\n",modwidth);
	    break;
    case 'b':			       /* -bypass filters*/
	    //~ if (--argc)
	      //~ sscanf(*(++argv),"%lf",&temp);
      bypass=1;
	    if (trace&&bypass)
	      printf("Filters are bypassed\n");
	    break;
    case 'c':			       /* -clock frequency*/
	    if (--argc)
	      sscanf(*(++argv),"%lf",&BBDclock);
	    if (trace)
	      printf("BBD clock set to %f.\n",BBDclock);
	    break;
    case 'g':			       /* -gain*/
	    if (--argc)
	      sscanf(*(++argv),"%lf",&gain);
	    if (trace)
	      printf("input gain is set to %f.\n",gain);
	    break;
    case 'p':			       /* -preset*/
	    if (--argc)
	      sscanf(*(++argv),"%lf",&temp);
    ...
    if (result != -1){
      char command[100];
      sprintf(command,"rm %s", outsfname,outsfname);
      system(command);
      }
 ...
 */
    ...
    outfd = sndlib_create(outsfname, inType, inFormat, newsrate, 1, comment);
    if (outfd == -1)
    ...
    int sound_bytes = outCountReal * 1 * mus_bytes_per_sample(outFormat);
