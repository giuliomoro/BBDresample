...
double BBDclock=68175; /*clock frequency Fs2*/
int bypass=0; /**/
int preset=0;
double gain=1;
...
double lfoGen(int channel, UWORD uTime, double sysFs){
    ...
    double famp=0.28; /*gain reduction of fast waveform*/
    switch (preset){
    case 0 : /*slow*/
        switch (channel){
        case 0 :
            return sin(pi2*Time*fslow+phs);
        case 1 : 
            return 0;
        case 2 :
            return 0;
    ...
    case 1 : /*chorus 1*/
        ...
        case 0:
            return sin(pi2*Time*fslow+phs);
        case 1:
            return sin(pi2*Time*fslow+phs+ph1);
        case 2:
            return sin(pi2*Time*fslow+phs+ph2);
        }
    case 2 : /*chorus 2*/
        switch (channel){
        case 0 :
            return sin(pi2*Time*fslow+phs) + famp*sin(pi2*Time*ffast+phf);
        case 1 :
            return sin(pi2*Time*fslow+phs+ph1) + famp*sin(pi2*Time*ffast+phf+ph1);
        case 2 : 
            return sin(pi2*Time*fslow+phs+ph2) + famp*sin(pi2*Time*ffast+phf+ph2);
        }
    case 3: /*fast*/
        switch (channel){
        case 0:
    ...
    *Time=startTime;
    ...
    while ( *Time-startTime >>Np < Nx ){
      ...
      if (*Time==nextBBDevent[ch]){
        ...
        bbd[ch][BBDindex[ch]]= washcompute(v*amp1) + noise()*NOISEAMP;/*noise*10^(-70/20)*/
        oldBBDevent[ch]=*Time;
      ...
      if (*Time==nextTevent){
        ...
        dhw = MIN(2*Npc-1,Npc*T2lfo);  /* Filter sampling period min(Fs,Fs')*/
        //~ dhw=Npc;
        dhbw = dhw*(1<<Na) + 0.5;     /* Fixed-point representation*/
        ...
        outmin=MIN(outmin,v*amp2);
        outmax=MAX(outmax,v*amp2);
                                      c++;
        *Y++=*Y+ amp2*v/rescale;
        //~ *Y+=.5;
        //~ *Y+=bbd[ch][rread];
    ...
    UHWORD LpScl, UHWORD Nmult, UHWORD Nwing)
{
    ...
    do {
      ...
      if (!init)
        ...
        Nout=SrcUD(X1,Y1+RECfilter_Blgt,&Time,Nx,Nwing,LpScl,Imp,ImpD,interpFilt,X2,channels,rescale,onboard,44100);
